...
window.ADSPANDABLE_CONFIG = {
    ...
    contentClick:false,
    // end mobile
    text: "<strong>The King Furniture Summer Sale is on now</strong><br/><br/>You'll love the thought of saving up to 50% off selected designs. Thoughtfully crafted for sumptuous comfort, superior durability & flexibility, there's a style & affordable King sofa to suit every Australian home. </br> So don't miss the King Furniture Summer Sale! ",
    ...
    cookieCap : true,
    clickTag: 'http://ad.doubleclick.net/ddm/clk/287250162;114290130;p',
    ...
    */
    ...
    actualViewClientTrack: 'http://ad.doubleclick.net/ad/N799.1124336AVIDGLOBALMEDIA.COM/B8422711.114290130;sz=1x1;ord='+Math.floor(Math.random()*100000000)+'?',
    ...
    customFilterJs:(function(){
        ...
        // ibtimes tech section only and user iframe tags
        if (dd.indexOf('ibtimes') >- 1 && ifrHead.indexOf('media-culture')<0) {
         rt = false;
        }

        //sheknows home and living section only
        if (dd.indexOf('sheknows') >- 1 && lh.indexOf('home-and-living')<0) {
            rt = false;

        }

        // timesofindia.indiatimes life style section only
        if (dd.indexOf('indiatimes') >- 1 && lh.indexOf('life-style')<0) {
            rt = false;

        }
        // stylecaster life style section only
        if(dd.indexOf('stylecaster')>-1 && (lh.indexOf('lifestyle')<0 && lh.indexOf('love-dating')<0 && lh.indexOf('health-fitness')<0 && lh.indexOf('for-women')<0 && lh.indexOf('beauty')<0 && lh.indexOf('diy')<0 && lh.indexOf('food-and-drink')<0 && lh.indexOf('rule-breakers')<0) && head.indexOf('lifestyle')<0){
            rt = false;
        }
