...
(function(){
    ...
    function hashChangedCallback(newHash, oldHash) {
        ...
        if (newHash.indexOf('-trueview') >- 1){
            ...
            if(campaign.indexOf('-cp')<0) {
                ...
                if (typeof jwplayer('video-container') != 'undefined') {
                    if (jwplayer('video-container').getState() == "PLAYING") {
        ...
        //window.slideTimer = new globalTimer();
           // slideTimer.start('playsuit');
        ...
        if (newHash.indexOf('-SEPSLIDE-start') >- 1){
            ...
            /**display radomly social feed or article;**/

            //var articleArray = ['what_to_wear','new_neutrals_1','feel_natural','get_in_nude','LDN','hm_debutes','Lara_Stone_1','Lara_Stone_2','new_neutrals_2'];
            ...
            var t = setInterval(function(){
                if(typeof AdobeEdge !== 'undefined'){
                    ...
                    AdobeEdge.bootstrapCallback(function(compId){
                        ...
                        if(randomPick == 1){
                            window.showRandomFeed();
                            var p =setInterval(function(){
                                if(typeof SlidejQueryAnimater !== 'undefined') {
                                    SlidejQueryAnimater.stopTimer();
                                    clearInterval(p);
                                }
                            },400);
                        }
                            ...
                            var r = setInterval(function(){
                                if(typeof edgeStartAnimation.launchArticles !== 'undefined'){
                                    //pick radomlly article from 0 to 8;
                                    var articlePick = Math.floor(Math.random()*8);
                                    edgeStartAnimation.launchArticles(articlePick);
                                    SlidejQueryAnimater.stopTimer();
                                    clearInterval(r);
                                }
                            },400);
            ...
            /* function showArticle(){
             //check if Edge loaded
             var EdgeLoaded = setTimeout(function(){

             console.log(AdobeEdge.bootstrapCallback);

             if(AdobeEdge.bootstrapCallback === "function"){
             clearTimeout(EdgeLoaded);
             console.log('defined');
             AdobeEdge.bootstrapCallback(function(compId){
             //pick radomlly article from 0 to 8;
             var articlePick = Math.floor(Math.random()*8);
            ...
            if(!isMobile.any){
                ...
                // openVideo('video-640.mp4','intro',true,'video-640.mp4.jpg');//it takes forth parameter for video poster image as eg. ('myVideo.mp4','intro',true,'myPosterImage.jpg/png/gif')
            }

            // content play only - please ignore for anything else
            if(campaign.indexOf('-cp')>-1){
                if(typeof jwplayer('video-container') != 'undefined' && (jwplayer('video-container').getState() == "PAUSED" || jwplayer('video-container').getState() == "PLAYING" || jwplayer('video-container').getState() == "IDLE" || jwplayer('video-container').getState() == "BUFFERING")) {
                    if(!isMobile.any) {
                        jwplayer('video-container').play(true);
                    }
        ...
        if (newHash.indexOf('-SEPSLIDE-stop') >- 1){
            ...
            if(campaign.indexOf('-cp')<0) {
                ...
                // if content play, sto video but don't remove
                if ($('#video-container').length>0){
    ...
    $(document).ready(function(){
        ...
        if(isMobile.any){
            ...
            if ($('#video-container').length>0 && $('.mobileLaunchVideo').length>0){
                ...
                $('#arrow-left,#arrow-right,#videoIconRight,#videoIconLeft').click(function(){
                    ...
                    openVideo('','intro',true,'sth.jpg');//it takes forth parameter for video poster image as eg. ('myVideo.mp4','intro',true,'myPosterImage.jpg/png/gif')
        ...
        var elementsId;
        var previousId;
        $( "#instagramButton,#twitterButton,#articleButton" ).click(function(e) {

            window.slideShowAuto = false;
            ...
            $(this).attr('title', currentButton +'button pressed');
            trackingCall('-events-mousepress-evente--soc-'+$(this).attr('title')+'-soe--cos-'+e.pageX+'-'+e.pageY+'-coe-/'+campaign+'/'+getKeywordInfo.info);

            //check which widget is displayed now


            $('.widget').each(function(index,element){
                elementsId = element.id;
                if(elementsId!=currentId){
                    $(element).css('left','-70%');
        ...
        $( ".backButton" ).click(function(e) {
            window.slideShowAuto = true;
           // slideTimer.stop();
            var currentCardParent = $(this).parent();
            var currentCard = currentCardParent.attr('id');

            //tracking mousepress
            //var currentButton = currentId;
            $(this).attr('title', 'back button of card '+currentCard +' pressed ');
            trackingCall('-events-mousepress-evente--soc-'+$(this).attr('title')+'-soe--cos-'+e.pageX+'-'+e.pageY+'-coe-/'+campaign+'/'+getKeywordInfo.info);
        ...
        var videoPlayer = function(video,posterImage,videoPrettyName){
            ...
            if(!isMobile.any){
                ...
                // content play
                if(campaign.indexOf('-cp')>-1 && location.hash.indexOf('SEPSLIDE-start')<0) {
                    jwplayer('video-container').pause(true);
                }
            }
            ...
            if(campaign.indexOf('-cp')>-1){
                ...
                $('body').on('mouseover',function(){
                    jwplayer('video-container').setMute(false);
                });
...
// pass in edge dom event,jquery dom element
    ...
    //show radomly facebook,twitter or instargram feed after 2 sect

    window.showRandomFeed = function(){

        var feedsId = ['instagramFeed','twitterFeed'];
        var timerOutDelay = 2000;//2s
        var timerOut = setTimeout(function() {
            ...
            //random pick
            var randomPick = Math.floor(Math.random()*feedsId.length);
            currentId = feedsId[randomPick];
            previousId = feedsId[randomPick];


            window.slideShowAuto = false;
            //start counting Dwelling time
           // slideTimer.start(currentId);

            //animate
            $("#"+feedsId[randomPick]).animate({
                left: '0'
            }, 200, "linear", function() {
