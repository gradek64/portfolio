...
(function () {
    ...
    var processMessage = function (event) {
        if (event.data == "is in view") {
            ...
            if(contentCard && !unitClosed && (typeof playerInstance=='object' && playerInstance.hasOwnProperty('getState') && playerInstance.getState() == "paused")){
            ...
            if(contentCard && !unitClosed && (typeof playerInstance=='object' && playerInstance.hasOwnProperty('getState') && playerInstance.getState() == "playing")){
                if(campaign.indexOf('sky')>-1){
                    playerInstance.play(true);
                }
            }
...
// mobile video controls
    ...
    trackingCall = function (info) {
        ...
        if (state === 'click') {
            ...
            pSrc = "https://reports.adspdbl.com/analytics.gif?ran=" + ran + "&type=event&state=" + state + "&audience=" + window.aud + "&campaign=" + campaign + "&win_price="+ win_price +"&spartanUnitId="+spartanid+"&general=" + general + "&click_title=" + clickTitle + "&resolution=" + $(window).width() + "x" + $(window).height() + "&keyword_data=" + keywordStr + "&domain=" + completeDomain + "&path=" + completeUrlformatted;
        }
        if (pSrc.length > 2000) {
                    ...
                    if(!defined){
                            defined = true;
                                ...
                                var f = setInterval(function(){
                                        if(typeof edgeStartAnimation.animStart === 'function'){
                                            clearInterval(f);
                                            edgeStartAnimation.animStart();
                                        }
                                },100 );
