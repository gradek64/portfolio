        ...
        shell : {
            ...
            mpfourConvert : {
                ...
                command : [
                    ...
                    'for f in $(ls -1 src/assets/*.mp4 | grep -v 640.mp4); do ffmpeg -i "$f" -b:v 250k -vf scale=640:360 -vcodec h264 -acodec aac -strict -2 ${f%.*}-640.mp4 -y; done'
                ].join('&&')
            },
            webmConvert : {
                options : {
                ...
                command : [
                    ...
                    'for f in $(ls -1 src/assets/*.mp4 | grep -v 640.mp4); do ffmpeg -i "$f" -vf scale=640:360 -vcodec libvpx -acodec libvorbis -strict -2 -crf 25 -b:v 250k -q:a 100 ${f%.*}-640.webm -y; done'
                ].join('&&')
            },
            mpegConvert : {
                options : {
