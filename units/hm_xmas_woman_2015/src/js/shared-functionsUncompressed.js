    ...
    function hashChangedCallback(newHash, oldHash) {
        ...
        if (newHash.indexOf('-SEPSLIDE-start') > -1) {
            ...
            if ($('#video-section').length > 0) {
                ...
                if (typeof jwplayer() == 'undefined') {
                    openVideo('HM_6125_HP2_UK_30_sec-640.mp4','intro',true,'HM_6125_HP2_UK_30_sec-640.jpg');
                }
                else {
                    // loop until video is ready to play
                    tmPlayerReadyTimer = '';
                    tmPlayerReady = function () {
                        if (typeof jwplayer() != 'undefined' && (jwplayer().getState() == "paused" || jwplayer().getState() == "idle") || vastadPaused) {
    ...
    var videoPlayer = function (video, posterImage, videoPrettyName, introVideo) {
        ...
        if (tubemogul) {
            ...
            jwplayer('video-container').setup({
                ...
                stretching: 'uniform',
                sharing: {"link": "http://content.jwplatform.com/previews/cE6acybV-UHthMnzO"},
                mute: true
        ...
        $('body').delegate("#video-container_sharing", "mouseover", function() {
            $('#closer').hide();
        });
        $('body').delegate("#video-container_sharing", "mouseout", function() {
            $('#closer').show();
        });
        ...
        jwplayer().onTime(function (e) {
            ...
            if (tubemogul || hostedVastVideo) {
                if (videoMobileDevices && !playAdonPlay) {
                    //console.log('here');
