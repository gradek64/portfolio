    ...
    grunt.task.registerTask('createTag', 'Creates a wrapped vast tag if necessary', function() {
      ...
      fs.readFile('src/js/shared-functionsUncompressed.js',"utf-8", function (err,data) {
        ...
        if(source!== null){
          ...
          appendTag = appendTag.replace(/var source = .*?";/,'var source = "https://reports.adspdbl.com/vast-tag-generator/?rdr='+source.replace('timestamp','"+Math.floor(Math.random() * 10000000000)+"')+'&domain="+completeDomain+"&campaign="+campaign+"&path="+completeUrlformatted[1];//done');

          if(appendTag.length<20){appendTag=data;}
