...
(function () {
    ...
    var processMessage = function (event) {
        if (event.data == "is in view") {
            ...
            if(!nielsenFired){
              ...
              tag.src = 'https://pixel.adsafeprotected.com/rjss/st/64582/10685422/skeleton.js';
    ...
    if (contentplay || tubemogul) {
        ...
        if($('img[data-title="Main Image Tag"]').length>0) {
            ...
            $('#container').prepend('<img id="sound-icon" class="s-on" src="http://files.adspdbl.com/assets/sound-off.png" style="width:6.14%;height:13%;position:absolute;right:1%;top:1%;z-index:2000;cursor:pointer;display:none;"/>');
...
// mobile video controls
    ...
    postMessageTracking = function (tracking) {
        if(completeDomain.indexOf('seattlepi')>-1 || completeDomain.indexOf('beaumontenterprise')>-1 || completeDomain.indexOf('ctpost')>-1 || completeDomain.indexOf('chron.com')>-1 || completeDomain.indexOf('greenwichtimes.com')>-1 || completeDomain.indexOf('mysa.com')>-1 || completeDomain.indexOf('newstimes.com')>-1 || completeDomain.indexOf('sfgate.com')>-1 || completeDomain.indexOf('stamfordadvocate.com')>-1  || completeDomain.indexOf('timesunion.com')>-1){
          if(completeDomain.indexOf('www')<0){
            completeDomain = 'www.'+completeDomain;
          }
        }
    ...
    trackingCall = function (info) {
        ...
        if (state === 'click') {
            ...
            // not mobile
                ...
                pSrc = "http://reports.adspdbl.com/redirectclicks.php?redirectAdsp=" + encodeURIComponent(clickRedirect) + "&ran=" + ran + "&type=event&state=" + state + "&audience=" + window.aud + "&campaign=" + campaign + "&win_price="+ win_price +"&spartanUnitId="+spartanid+"&general=" + general + "&click_title=" + clickTitle + "&resolution=" + $(window).width() + "x" + $(window).height() + "&keyword_data=" + keywordStr + "&domain=" + completeDomain + "&path=" + completeUrlformatted;
            }
        }
        else {
            pSrc = "http://reports.adspdbl.com/analytics.gif?ran=" + ran + "&type=event&state=" + state + "&audience=" + window.aud + "&campaign=" + campaign + "&win_price="+ win_price +"&spartanUnitId="+spartanid+"&general=" + general + "&click_title=" + clickTitle + "&resolution=" + $(window).width() + "x" + $(window).height() + "&keyword_data=" + keywordStr + "&domain=" + completeDomain + "&path=" + completeUrlformatted;
        }
        if (pSrc.length > 2000) {
    ...
    var videoPlayer = function (video, posterImage, videoPrettyName, introVideo) {
        ...
        if (tubemogul) {
            ...
            playerInstance.setup({
                ...
                // advertising: {"source":"http://etc","client": "vast"},
                playlist: [{
                    image: videoPosterImage,
                    sources: [
                        /*{file: "rtmp://flash.adspdbl.com/cfx/st/mp4:" + campaign + '/' + video},
        ...
        if (contentCard && videoMobileDevices) {
            if (isMobile.android.device && bowser.chrome) {
                $('#container').prepend('<img id="sound-icon" class="s-off" src="http://files.adspdbl.com/assets/sound-off.png" style="width:6.14%;height:13%;position:absolute;right:1%;top:1%;z-index:2000;cursor:pointer;display:none;"/>');
            }
        }
        ...
        if($('#sound-icon').length>0) {
            $('#sound-icon').css({
                ...
                if ($(this).hasClass('s-on')) {
                    ...
                    $(this).attr('src', 'http://files.adspdbl.com/assets/sound-on.png');
                }
                else {
                    playerInstance.setMute(true);
                    $(this).removeClass('s-off').addClass('s-on');
                    $(this).attr('src', 'http://files.adspdbl.com/assets/sound-off.png');
                }
            });
        }
