...
(function(){
    ...
    function hashChangedCallback(newHash, oldHash) {
        ...
        if (newHash.indexOf('-SEPSLIDE-start') >- 1){
            ...
            var edgeReady = function(){
                if(typeof edgeStartAnimation == "object" && typeof edgeStartAnimation.startAnimation == 'function') {
                    clearTimeout(edgeTo);
                    edgeStartAnimation.startAnimation();
                }
                else {
                    edgeTo = setTimeout(function(){edgeReady();},100);
                }
            };
            edgeReady();
            ...
            if(!isMobile.any){
               ...
               openVideo('oliving-video-640.mp4','intro',true,'oliving-video-640.jpg');//it takes forth parameter for video poster image as eg. ('myVideo.mp4','intro',true,'myPosterImage.jpg/png/gif')
            }
    ...
    $(document).ready(function(){
        ...
        var videoPlayer = function(video,posterImage,videoPrettyName){
            ...
            if (!isMobile.any){
                jwplayer('video-container').onDisplayClick(function(){if(!videoClickState && (jwplayer('video-container').getState()=="PAUSED" || jwplayer('video-container').getState()=="PLAYING")){videoClickState = true;trackingCall('-redirects-'+videoClick+'-redirecte--events-Click-evente- -soc-Video-soe-/'+campaign+'/-cos-0-coe-/'+getKeywordInfo.info);}else{}});
            }
            // play if not mobile
            if(!isMobile.any){
                jwplayer('video-container').play();
            ...
            if (lohh.indexOf('bounty')<0 && lohh.indexOf('ibtimes')<0 && lohh.indexOf('techweekeurope.co.uk')<0 && lohh.indexOf('timeout')<0 && lohh.indexOf('carsite')<0 && lohh.indexOf('autovillage')<0 && lohh.indexOf('madeformums')<0 && lohh.indexOf('babyexpert')<0 && lohh.indexOf('newsnow')<0 && lohh.indexOf('avforums')<0){
