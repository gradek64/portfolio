        ...
        shell : {
            ...
            mpfourConvert : {
                ...
                command : [
                    ...
                    'for f in $(ls -1 src/assets/*.mp4 | grep -v 640.mp4); do ffmpeg -i "$f" -vf scale=640:360 -vcodec h264 -acodec aac -strict -2 ${f%.*}-640.mp4 -y; done'
                ].join('&&')
            },
            webmConvert : {
                options : {
            ...
            hlsConvert : {
                ...
                command : [
                    ...
                    'for f in $(ls -1 src/assets/*.mp4 | grep -v 640.mp4); do ffmpeg -i "$f" -vcodec libx264 -acodec libfaac -hls_list_size 0 -hls_time 10 -crf 25 -b:v 250k -q:a 100 ${f%.*}-640.m3u8 -y; done'
                ].join('&&')
            },
            createPoster : {
                options: {
