...
(function () {
    ...
    // MUTED publishers
    var mutedPublisher = (lohh.indexOf('bounty') < 0 && lohh.indexOf('ibtimes') < 0 && lohh.indexOf('techweekeurope.co.uk') < 0 && lohh.indexOf('timeout') < 0 && lohh.indexOf('carsite') < 0 && lohh.indexOf('autovillage') < 0 && lohh.indexOf('madeformums') < 0 && lohh.indexOf('babyexpert') < 0 && lohh.indexOf('newsnow') < 0 && lohh.indexOf('avforums') < 0 && lohh.indexOf('thelocal') < 0 && lohh.indexOf('game-debate') < 0 ? false: true);


    var tubemogul = false,
    contentplay = false,
    mobAutoInitPause = false,
    playAdonPlay = false,
    vastadPaused = false,
    contentPlayhostedHasplayed = false,
    vastAdPausedOnce = false,
    sound = false,
    contentPlayNonHostedInit = false,

    // change this variable to false if you don't want to immediately launch with a video
    launchWithVideo = true,


    youTubeCP = false,
    youTubePlayer = {},
    youtubeFirstPlay = false,
    youTubeFirstQuart = false,
    youTubeHalfway = false,
    youTubeThirdQuart = false;


    // ----- for collective youtube videos
    if(youTubeCP) jwplayer = function(){
        return {
            getState:function(){},
            pause:function(){},
            play:function(){},
    ...
    // source of vast tag
    var source = "http://a.collective-media.net/pfadx/cnu.avid/simple_micellar;sz=1x1;ord=" + Math.floor(Math.random() * 10000000000) + ";dcmt=text/xml";
    //var source = "";
    if (campaign.indexOf('-cp') > -1) {
        // uncomment this line if not vast tag
        youTubeTag = '3QoHssfZjbg';
    ...
    // global unit in view
    window.currentlyInView = false;
    window.currentlyExpanded = false;


    var processMessage = function (event) {
        if (event.data == "is in view") {
            window.currentlyInView = true;
        }
...
// mobile video controls
    ...
    function hashChangedCallback(newHash, oldHash) {
        ...
        if (newHash.indexOf('-SEPSLIDE-start') > -1) {
            ...
            if ($('#video-section').length > 0) {
                ...
                if (videoMobileDevices && (tubemogul || contentplay)) {
                    postMessageTracking('firstClick');
                    $('#mle-oly').remove();
                }
                // youtube video
                if (youTubeCP) {
                    youTubeTriggered = false;
                    ytbePlayer = function() {
                        if (typeof youTubePlayer.playVideo != "function" && !youTubeTriggered){
                            openVideo('', 'intro', true);
                            youTubeTriggered = true;
                        }
                        else {
                            if (typeof youTubePlayer.getPlayerState == 'function' && youTubePlayer.getPlayerState() != 3 && youTubePlayer.getPlayerState() != -1) {
                    ...
                    // only for launching with video units
                    if (launchWithVideo){
                        if (typeof jwplayer() == 'undefined') {
                            openVideo('', 'intro', true);
                        }
    ...
    var videoPlayer = function (video, posterImage, videoPrettyName) {
        ...
        if (tubemogul) {
            ...
            onPlayerStateChange = function(event){
                ...
                var youtubeDuration = youTubePlayer.getDuration();
                var youtubeFirstQuartNum = Math.round(youtubeDuration/4);
                var youtubeHalfwayNum = Math.round(youtubeDuration/2);
                var youtubeThirdQuartNum = Math.round((youtubeDuration/4)*3);
                // on ended
                if (event.data == 0) {
                    //ended
                    trackingCall('-events-mousepress-evente--soc-Youtube ad complete ' + type + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                    postMessageTracking('closeUnit');
                    clearInterval(youtubeOntime);
                }
                else if(event.data == 2){
                    // paused
                    trackingCall('-events-mousepress-evente--soc-Youtube ad paused ' + type + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                    clearInterval(youtubeOntime);
                    ...
                    // playing
                    if(!youtubeFirstPlay) {
                        trackingCall('-events-mousepress-evente--soc-Youtube ad play ' + type + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                        youtubeFirstPlay = true;
                    }
                    youtubeOntime = setInterval(function(){
                        if(!youTubeFirstQuart && youTubePlayer.getCurrentTime()>youtubeFirstQuartNum){
                            youTubeFirstQuart = true;
                            trackingCall('-events-mousepress-evente--soc-Youtube First Quartile ' + type + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                        }
                        if(!youTubeHalfway && youTubePlayer.getCurrentTime()>youtubeHalfwayNum){
                            youTubeHalfway = true;
                            trackingCall('-events-mousepress-evente--soc-Youtube Halfway ' + type + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                        }
                        if(!youTubeThirdQuart && youTubePlayer.getCurrentTime()>youtubeThirdQuartNum){
                            youTubeThirdQuart = true;
                            trackingCall('-events-mousepress-evente--soc-Youtube Third Quartile ' + type + '-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                        }
            ...
            jwplayer('video-container').setup({
                ...
                primary: (function () {
                    var rt = 'flash';
                    if (completeDomain.indexOf('bounty') > -1) {
                        rt = 'html5';
                    }
                    return rt;
                }()),
        ...
        if (!youTubeCP) {
            ...
            jwplayer().onComplete(function () {
                // log completed video
                trackingCall('-events-mousepress-evente--soc-' + videoPrettyName + ' - ver-soe--cos--coe-/' + campaign + '/' + getKeywordInfo.info);
                if (contentplay && !hostedVastVideo) {
                    postMessageTracking('closeUnit');
                }
            ...
            if (tubemogul || hostedVastVideo) {
                ...
                // completed video (don't trust oncomplete its unreliable)
            ...
            if (tubemogul || contentplay) {
                jwplayer("video-container").onAdTime(function (e) {
                    ...
                    /*if (!window.currentlyInView && !currentlyExpanded) {
                     var vc = $('#video-container video').get(0);
                     vc.pause();
                     }

                     if (window.currentlyInView && !window.currentlyExpanded) {
                     var vc = $('#video-container video').get(0);
                     vc.pause();
                     }*/
    ...
    // if its a video unit
    if ($('#video-section').length > 0 && !youTubeCP && launchWithVideo) {
        if (!tubemogul) {
            openVideo('', 'intro', true)
        }
        else {
    ...
    $(document).ready(function () {
        ...
        $('body').delegate('.campaign-link', 'click', function (e) {
            var frame = '';
